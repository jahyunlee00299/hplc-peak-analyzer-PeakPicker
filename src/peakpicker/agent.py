...
from .qc_checker import QcChecker, check_calibration_quality
...
class LCQuantAgent:
    ...
    def __init__(
        ...
        # Audit calibration provenance once, at load time (see qc_checker).
        self.calibration_warnings = check_calibration_quality(
            self._compounds, self._config.qc
        )
    ...
    def _scan(self, data_dir: str) -> List[SampleMeta]:
        ...
        fed = sum(1 for s in samples if s.all_factors().get("is_fed", False))
